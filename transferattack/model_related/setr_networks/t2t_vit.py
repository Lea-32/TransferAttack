...
class T2T_ViT(nn.Module):
    ...
    def forward_features(self, x):
        ...
        for idx, blk in enumerate(self.blocks):
            x = blk(x)
            out_temp = self.norm(x)
            block_heads.append(out_temp)

        x = self.norm(x)
        return x[:, 0], block_heads

    def forward(self, x, get_average=False):
        x, block_heads = self.forward_features(x)
        if get_average:
            return torch.mean(torch.stack([self.head(x[:, 0]) for x in block_heads], 0), dim=0)
        x = self.head(x)
        return x
